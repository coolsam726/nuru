...
async def _lifespan(app: FastAPI):
    ...
    async with _get_session() as session:
        if not (await session.exec(sm_select(User))).first():
            session.add_all([
                User(name="Alice Kamau",   email="alice@example.com",  role="admin",  active=True),
                User(name="Bob Mwangi",    email="bob@example.com",    role="editor", active=True),
                User(name="Carol Ochieng", email="carol@example.com",  role="viewer", active=False),
                User(name="David Otieno",  email="david@example.com",  role="editor", active=True),
                # Seed more users to demonstrate pagination and search:
                User(name="Eve Njeri",     email="eve@example.com",    role="viewer", active=True),
                User(name="Frank Oduor",   email="frank@example.com",  role="viewer", active=True),
                User(name="Grace Achieng", email="grace@example.com",  role="viewer", active=True),
                User(name="Heidi Onyango", email="heidi@example.com", role="editor", active=False),
                User(name="Ivan Otieno",   email="ivan@example.com",   role="viewer", active=True),
                User(name="Judy Wanjiku",  email="judy@example.com",   role="admin",  active=True),
                User(name="Karl Mwangi",   email="karl@example.com",   role="viewer", active=True),
                User(name="Leo Kamau",     email="leo@example.com",    role="viewer", active=True),
            ])

        if not (await session.exec(sm_select(Order))).first():
            session.add_all([
                Order(order_number="ORD-001", customer="Alice Kamau",
                      status="delivered",  total=4500.00),
                Order(order_number="ORD-002", customer="Bob Mwangi",
                      status="shipped",    total=1200.50),
                Order(order_number="ORD-003", customer="Carol Ochieng",
                      status="pending",    total=890.00),
                Order(order_number="ORD-004", customer="David Otieno",
                      status="processing", total=3300.75),
                Order(order_number="ORD-005", customer="Eve Njeri",
                      status="cancelled",  total=150.00),
                Order(order_number="ORD-006", customer="Frank Oduor", status="pending",    total=2200.00),
                Order(order_number="ORD-007", customer="Grace Achieng", status="pending",    total=780.00),
                Order(order_number="ORD-008", customer="Heidi Onyango", status="processing", total=4100.00),
                Order(order_number="ORD-009", customer="Ivan Otieno", status="shipped",    total=670.25),
                Order(order_number="ORD-010", customer="Judy Wanjiku", status="delivered",  total=540.00),
                Order(order_number="ORD-011", customer="Karl Mwangi", status="pending",    total=1300.00),
                Order(order_number="ORD-012", customer="Leo Kamau", status="cancelled",  total=250.00),
                Order(order_number="ORD-013", customer="Alice Kamau", status="processing", total=900.00),
                # Populate 50 more records to demonstrate pagination:
                *[Order(order_number=f"ORD-{100+i:03d}", customer="Customer "+str(i),
                       status="pending", total=100.00 + i*10) for i in range(1, 51)]
            ])
...
_servers = [
    {"id": 1, "hostname": "web-01",   "region": "eu-west", "status": "healthy", "cpu": 23},
    {"id": 2, "hostname": "web-02",   "region": "eu-west", "status": "healthy", "cpu": 41},
    {"id": 3, "hostname": "db-01",    "region": "us-east", "status": "warning", "cpu": 87},
    {"id": 4, "hostname": "cache-01", "region": "us-east", "status": "healthy", "cpu": 12},
]
...
class UserResource(Resource):
    ...
    row_actions = [
        ...
        Action("reset_email", label="Reset email", handler="reset_email",
               style="default",
               form_fields=[
                   fields.Email("new_email", "New email address",
                                required=True, placeholder="user@example.com"),
               ]),
    ]

    list_actions = [
        Action("export_csv", label="Export CSV", handler="export_csv",
               style="secondary",
               icon="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"),
        Action("send_notice", label="Send Notice", handler="send_notice",
               style="warning",
               form_fields=[
    ...
    table_columns = [
        ...
        columns.Badge("role", "Role",  colors={
            "admin":  "purple",
            "editor": "blue",
            "viewer": "gray",
        }),
...
class OrderResource(Resource):
    ...
    form_actions = [
        Action("mark_shipped", label="Mark as Shipped", handler="mark_shipped",
               placement="header", style="primary",
               confirm="Mark this order as shipped?"),
        Action("cancel_order", label="Cancel Order", handler="cancel_order",
               placement="header", style="danger",
               form_fields=[
                   fields.Textarea("reason", "Cancellation reason",
                                   required=True,
                                   placeholder="Explain why this order is being cancelled..."),
               ]),
        Action("add_note", label="Add Note", handler="add_note",
               placement="inline", style="default",
               form_fields=[
                   fields.Textarea("note", "Internal note",
                                   required=True,
...
class ServerResource(Resource):
    ...
    async def get_list(self, *, page=1, per_page=25, search=None,
                       sort_by=None, sort_dir="asc", filters=None):
        data = list(_servers)
        if search:
            q = search.lower()
...
class ProductResource(Resource):
    ...
    row_actions = [
        Action("restock", label="Restock", handler="restock_product",
               style="success", icon="M12 4v16m8-8H4",
               form_fields=[
